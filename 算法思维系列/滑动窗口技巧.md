...
2、我们先不断地[增加 right]() 指针扩大窗口 [left, right]，直到窗口中的[字符串符合要求]()（包含了 T 中的所有字符）。

3、此时，我们停止增加 right，转而不断[增加 left 指针缩小窗口]() [left, right]，直到窗口中的字符串不再符合要求（不包含 T 中的所有字符了）。同时，每次增加 left，我们都要更[新一轮结果]()。

4、重复第 2 和第 3 步，直到 right 到[达字符串 S 的尽头]()。
...
可以用两个[哈希表当作计数器]()解决。用一个哈希表 needs 记录字符串 t 中包含的字符及出现次数，用另一个哈希表 window 记录当前「窗口」中包含的字符及出现的次数，如果 window 包含所有 needs 中的键，且这些键对应的值都大于等于 needs 中的值，那么就可以知道当前「窗口」符合要求了，可以开始移动 left 指针了。
...
读者也许认为嵌套的 while 循环复杂度应该是平方级，但是你这样想，while 执行的次数就是[双指针 left 和 right 走的总路程，最多是 2M]() 嘛。
...
其中 window 的数据类型可以视具体情况而定，比如上述题目都使用[哈希表充当计数器]()，当然你也可以用一个数组实现同样效果，因为我们只处理英文字母。
